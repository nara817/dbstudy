...
-- 수강신청(ENROLL) 테이블 생성
-- 2) 외래키에 지정할 수 있는 외래키 장착해보기
CREATE TABLE ENROLL_TBL (
    ENR_NO   NUMBER           NOT NULL,
    STU_NO   VARCHAR2(5 BYTE) NOT NULL,
    SJT_CODE VARCHAR2(1 BYTE) NOT NULL,
    CONSTRAINT PK_ENROLL PRIMARY KEY(ENR_NO),
    CONSTRAINT FK_ENROLL_STUDENT FOREIGN KEY(STU_NO) REFERENCES STUDENT_TBL(STU_NO) ON DELETE CASCADE, -- 2)
    CONSTRAINT FK_ENROLL_SUBJECT FOREIGN KEY(SJT_CODE) REFERENCES SUBJECT_TBL(SJT_CODE) ON DELETE CASCADE -- 2)
);
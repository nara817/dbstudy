...
BEGIN
    ...
    ELSIF DELETING THEN --- 수정 후 동작할 작업
...
/*
    ...
    1. INESERT, UPDATE, DELETE 수행 이전의 정보를 임시 저장하고 있는 테이블이다.
    2. :OLD 방식으로 호출한다.
...
/*
    트리거 실습
    사원 테이블에서 삭제할 사원정보를 퇴사자 테이블로 삽입하는 RETIRE_TRIG(은퇴) 생성하기
    
    작업 순서
    1. 사원 테이블(EMPLOYEES)의 구조를 복사하여 퇴사자 테이블(RETIRES)을 생성한다.
    2. 퇴사자 테이블(RETIRES)에 퇴사번호(RETIRE_ID), 퇴사일자(RETIRE_DATE) 칼럼을 추가한다.

/*
    ...
    3. 형식
        CREATE PROCEDURE
        CREATE [OR REPLACE] PROCEDURE 프로시저명[(매개변수)] -- ()괄호 안에는 매개변수가 들어 감, 매개 변수가 없다면 괄호 자체가 생략 될 수 있음!
        > OR REPLACE이 DROP을 대신할 수 있음(만들거나 바꾸거나)
        IS -- AS도 가능
            변수선언
        BEGIN 
            본문
        [EXCEPTION
            예외처리]
        END;
    4. 작성된 프로시저는 EXCUTE 프로시저(); 형식으로 실행한다. > 실행할 땐 괄호 생략 불가!
*/

-- 프로시저1 정의(만들기) >> 출력은 안됨
CREATE OR REPLACE PROCEDURE PROC1 -- 괄호생략 매개변수 없음
IS -- AS도 상관 없음!
BEGIN
...
-- 프로시저1 호출(실행하기)
EXECUTE PROC1();  -- EXCUTE 줄여 쓸 수 있음!

-- 프로시저2 정의(만들기) : 사원번호가 100인 사원의 FIRST_NAME, LAST_NAME, SALSRY를 조회하는 프로시저
...
-- 프로시저3 정의(만들기) : 사원번호를 전달하면 해당사원의 FIRST_NAME, LAST_NAME, SALSRY를 조회하는 프로시저
-- 사용할 매개변수 : 입력 파라미터
-- 1. 프로시저로 값을 전달 할때 사용한다.
-- 2. 형식 : 변수명 IN 타입
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_PUTOUT.PUT_LINE('예외가 발생했습니다.');
END;
...
-- 프로시저4 정의(만들기)
-- 사원번호가 100인 사원의 FIRST_NAME, LAST_NAME을 출력 파라미터로 반환하는 프로시저(반환값을 받아옴 RETURN)
...
BEGIN
    SELECT FIRST_NAME, LAST_NAME
      INTO FNAME, LNAME -- 이 곳에서 출력 파라미터 값이 저장된다
      FROM EMPLOYEES
    WHERE EMPLOYEE_ID = 100;
END;

-- 프로시저4 호출(실행하기)
-- FNAME과 LNAME값을 저장할 변수를 선언한 뒤, 프로시저에 전달해야한다.
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
    LNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    PROC4(FNAME, LNAME); -- PLSQL 내부에서 프로시저를 호출할 때는 EXCUTE를 생략해야 한다.
    DBMS_OUTPUT.PUT_LINE(FNAME || ',' || LNAME);
END;

-- 프로시저5. 사원번호를 전달하면, 해당 사원의 FIRST_NAME이 출력 파라미터 FNAME에 저장되도록 작성.
-- 없는 사원번호가 전달되면 FNAME에 'Noname'이 저장되도록

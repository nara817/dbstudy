...
DESCRIBE USER_INDEXES
  SELECT EMP, INDEX_NAME, COLUMN_NAME
...
SELECT EMPID, EMPNAME, SALARY, DEPTID
  FROM EMPLOYEE_TBL
 WHERE DEPTNAME = (SELECT MAX(SALARY)
                   FROM EMPLOYEE_TBL);

-- 문제2
SELECT EMPID, EMPNAME, SALARY, DEPTID, DEPTNAME
  FROM EMPLOYEE_TBL
 WHERE DEPT IN (SELECT DEPT_NAME
                  FROM DEPARTMENT_TBL
                 WHERE DEPT_NAME = '총무부');
...
CREATE TABLE BOARD_TBL (
    ...
    HIT          NUMBER,
    CREATE_DATE  DATE,
    CONSTRAINT BOARD_NO  PRIMARY KEY(BOARD_NO)
);

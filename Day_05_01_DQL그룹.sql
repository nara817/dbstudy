...
SELECT DEPARTMENT_ID-- +중복제거 DISTINT없이도 가능 
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;
   
--2. 동일한 DEPARTMENT_ID를 그룹화하여 FIRST_NAME 과 DEPARTMENT_ID를조회하시오.(실패하는 쿼리문)
SELECT FIRST_NAME, DEPARTMENT_ID --  FIRST_NAME 칼럼이 GROUP BY벌에 없기 떄문에 실행되지 않는다.
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;
...
SELECT
       DISTINT DEPARTMENT_ID
     , COUNT(*) OVER(PARTITION BY DEPARTMENT_ID) AS 부서별사원수--DEPARTMENT_ID별로 모아서 COUNT해라
     , SUM(SALARY) OVER(PARTITION BY DEPARTMENT_ID) AS 부서별연봉합
     , AVG(SALARY) OVER(PARTITION BY DEPARTMENT_ID) AS 부서별연봉편균

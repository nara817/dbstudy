--- 학생 이름, 담당교수 이름 조회하기
-- SELECT 학생테이블의 학생이름, 교수테이블의 담당교수 이름
--   FROM 교수 P 별명 INNER JOIN 학생 별명
--     ON 교수테이블의 담당교수번호 = 학생테이블의 학생번호

SELECT S.S_NAME, P.P_NAME
  FROM PROFESSOR_TBL P INNER JOIN STUDENT_TBL S -- 드라이브테이블/드리븐테이블
    ON P.P_NO = S.P_NO; -- PK == STUDENT_TBL 담당교수번호
    
SELECT S.S_NAME, P.P_NAME
  FROM PROFESSOR_TBL P, STUDENT_TBL S
  WHERE P.P_NO = S.P_NO;
-- SELECT 
--   FROM 
--     ON
--
-- 교수번호, 교수이름, 교수전공, 강의이름, 강의실을 조회하시오. ** 수정필요**
SELECT P.P_NO, P.P_NAME, P.P_MAJOR, L.L_NAME, L.L_LOCATION--** 테이블 명칭 알 수 없을땐 SELECT* 로 먼저 시작!
  FROM PROFESSOR_TBL P INNER JOIN LECTURE_TBL L
    ON P.P_NO = L.P_NO;
    
SELECT P.P_NO, P.P_NAME, P.P_MAJOR, L.L_NAME, L.L_LOCATION
  FROM PROFESSOR_TBL P, LECTURE_TBL L
 WHERE P.P_NO = L.P_NO;
 
 -- 학번, 학생이름, 수강 신청한 과목명을 조회하시오.
 -- 학생+수강신청 먼저 조인 > 과목 조인
SELECT S.S_NO, S.S_NAME, C.C_NAME
 FROM STUDENT_TBL S INNER JOIN ENROLL_TBL E
   ON S.S_NO = E.S_NO INNER JOIN COURSE_TBL C -- 학생+수강신청 조인 + 과목 조인
   ON E.C_NO = C.C_NO;
   

SELECT S.S_NO, S.S_NAME, C.C_NAME
  FROM STUDENT_TBL S, ENROLL_TBL E, COURSE_TBL C
 WHERE S.S_NO = E.S_NO
   AND E.C_NO = C.C_NO;
   
-- 모든 교수들의 교수이름, 교수전공, 강의이름을 조회하시오. (교수는 총 3명이다.)
SELECT P.P_NAME, P.P_MAJOR, L.L_NAME
  FROM PROFESSOR_TBL P LEFT OUTER JOIN LECTURE_TBL L
    ON P.P_NO = L.P_NO;
    
SELECT P.P_NAME, P.P_MAJOR, L.L_NAME
  FROM PROFESSOR_TBL P, LECTURE_TBL L
 WHERE P.P_NO = L.P_NO(+);